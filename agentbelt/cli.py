"""`agentbelt` CLI — the dev on-ramp.

  agentbelt init [path]      # scaffold a starter config (default: agentbelt.yaml)
  agentbelt check            # validate config + all providers (exit 0 ok / 1 errors)
  agentbelt serve            # run the OpenAI-compatible proxy (localhost:8088)
  agentbelt test             # replay the red-team corpus vs your config (exit 0 all blocked / 1 any allowed)
  agentbelt dash [path]      # render the audit-log JSONL as a terminal summary (AGENTBELT_AUDIT_LOG)

Config path resolves from AGENTBELT_CONFIG, else ./agentbelt.yaml. The proxy is unauthenticated by
...
_STARTER = """\
# Agentbelt config — edit to fit your agent, then `agentbelt check` and `agentbelt serve`.
agent: my-assistant
...
def _config_path() -> str:
    return os.environ.get("AGENTBELT_CONFIG", "agentbelt.yaml")
...
def _cmd_init(path: str) -> int:
    ...
    with open(path, "w") as f:
        f.write(_STARTER)
    print(f"wrote {path} — edit it, then `agentbelt check`")
    return 0


def _cmd_dash(path: str | None) -> int:
    """Render the audit-log JSONL as a terminal summary (read-only snapshot)."""
    path = path or os.environ.get("AGENTBELT_AUDIT_LOG")
    if not path:
        print(
            "no audit log — pass a path or set AGENTBELT_AUDIT_LOG "
            "(set it when running `agentbelt serve` to record decisions)",
            file=sys.stderr,
        )
        return 1
    if not os.path.exists(path):
        print(f"no audit log at {path} — run the proxy with AGENTBELT_AUDIT_LOG set to record decisions", file=sys.stderr)
        return 1
    from agentbelt.dash import render
...
def _cmd_test(cfg) -> int:
    ...
    from agentbelt.redteam import run, summary

    results = run(cfg)
    table = Table(title="Agentbelt red-team replay")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="agentbelt", description="Protective harness for conversational agents.")
    ...
    si.add_argument("path", nargs="?", default="agentbelt.yaml")
    ...
    sd.add_argument("path", nargs="?", default=None, help="audit-log path (default: $AGENTBELT_AUDIT_LOG)")
    ...
    from agentbelt.config import load_config
    from agentbelt.validate import validate

    path = _config_path()
    if not os.path.exists(path):
        print(f"no config at {path} — run `agentbelt init` or set AGENTBELT_CONFIG", file=sys.stderr)
        return 1
