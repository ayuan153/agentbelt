...
How Seatbelt gets from "clone and run" to "a dev plays with it in 60 seconds and adopts it." Phased
by priority, not by date. ✅ = shipped.

## The on-ramp we're optimizing for

```
pip install seatbelt-harness        # one install
seatbelt init                       # scaffold a config
# edit seatbelt.yaml
seatbelt check                      # validate, fail-fast
seatbelt serve                      # OpenAI-compatible proxy on :8088
...
- ✅ `pip install` packaging (`pyproject.toml`, `seatbelt-harness`) with a `seatbelt` console script.
...
- **Publish to PyPI** so `pip install seatbelt-harness` works for everyone (build with `python -m
  build`, upload via `twine`/Trusted Publishing). Tag releases; semantic versioning.
- **`pipx` / `uvx` support** so `uvx seatbelt-harness serve` runs it with zero env setup.
- **Container image** (`ghcr.io/.../seatbelt`) for `docker run` / sidecar deployment.
- A few **ready-made example configs** (support bot, RAG assistant, coding assistant) shipped in the
  package and listed by `seatbelt init --template <name>`.
...
- **In-process shim as decorators**: `@seatbelt.tool(tier="high")` around a tool fn, so embedding
  the harness is a one-import change for Python agents.
- **Optional npm package** (`@seatbelt/client`): a thin TypeScript client + CLI wrapper for
  Node/Deno agents. *Interop already works without it* (base_url) — npm is purely DX sugar (typed
  config, `npx seatbelt serve` via a bundled runtime, shim helpers).
...
- **npm scope**: thin client only, or a full Node-native re-implementation of the guards? The proxy
  ...
  is in demand.
- **Naming**: "Seatbelt" collides with a well-known offensive-security tool (GhostPack). The PyPI
  distribution is `seatbelt-harness`; consider a distinct public brand to avoid confusion.
